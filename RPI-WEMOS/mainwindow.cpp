        ...
        for (const QString& ruwBericht : berichten) {
            ...
            if (parseFout.error == QJsonParseError::NoError) {
                ...
                // SENSOR DATA
                    ...
                    if (id.isEmpty()) id = jsonObj["sensorld"].toString();
...
void MainWindow::on_pushButton_Update_Lichtkrant_clicked()
{
    ...
    QString zendBericht = "LICHTKRANT:" + nieuweTekst + "\n";

    socketComm->verzendData(zendBericht);

    QString tijd = QDateTime::currentDateTime().toString("hh:mm:ss");
    ui->textBrowser_Logboek->append(tijd + " - Verzonden: " + zendBericht.trimmed());
